...
const CONFIG_FILE = path.join(DATABASE_DIR, 'antilink.json');
...
function getDB() {
    try {
        ...
        console.error('❌ AntiLink database read error:', err.message);
        return {};
    }
}
...
function saveDB(data) {
    try {
        ...
        console.error('❌ AntiLink database save error:', err.message);
    }
}
...
// Group & Channel links only
// ═══════════════════════════════════════

function hasBlockedLink(text) {
    if (!text || typeof text !== 'string') return false;
    const regex = /(?:https?:\/\/)?(?:www\.)?(?:chat\.whatsapp\.com\/[A-Za-z0-9_-]+|whatsapp\.com\/channel\/[A-Za-z0-9_-]+)/i;
...
// ═══════════════════════════════════════
// MAIN ANTILINK EVENT ENGINE
// ═══════════════════════════════════════

async function processAntiLink(sock, msg) {
    try {
        ...
        if (!text || !hasBlockedLink(text)) return;
...
🚫 Group and Channel links are forbidden.

⚠️ Warning: ${warning}/3

⏳ Remaining: ${remaining}`,                                                            contextInfo: { mentionedJid: mentionList, ...style }
            }, { quoted: msg });
            return;
        }
...
Reason: Maximum Anti-Link warnings reached (3/3).`,
...
Admins must remove manually.`,
        ...
        console.error('❌ AntiLink processing error:', error.message);
    }
}
...
async function antilinkEvent(sock, update) {
    try {
        if (!update) return;
                                                                               if (update.messages) {
            for (const msg of update.messages) {
                await processAntiLink(sock, msg);                                  }
        }

        if (Array.isArray(update)) {
            for (const item of update) {
                if (item.update?.message) {
                    await processAntiLink(sock, {
                        ...item,
        ...
        console.error('❌ AntiLink event error:', error.message);
    }
}                                                                      
// ═══════════════════════════════════════
// ANTILINK COMMAND
...
async function antilinkCommand(sock, msg, args, jid) {
    try {
        ...
        if (!isAdmin(admins, sender)) {                                            return sock.sendMessage(jid, {
                text: '🚫 Only group administrators can use Anti-Link.'            }, { quoted: msg });
        }                                                              
        const db = getDB();
        ...
        const style = getCybernovaStyle();
                                                                               // RESET
        if (option === 'reset') {
            if (db[jid]?.enabled) {                                                    resetAllWarnings(jid);
                return sock.sendMessage(jid, {
                    text:
`✅ *WARNINGS RESET*

All Anti-Link warnings have been clear`,
                    contextInfo: style
                }, { quoted: msg });                                               } else {
                return sock.sendMessage(jid, {
                    text:
`⚠️ *ANTI-LINK NOT ACTIVE*                                              
Anti-Link is currently disabled.

Usage:
.antilink on
.antilink off                                                          .antilink reset`,
                    contextInfo: style
                }, { quoted: msg });
            }
        }
        ...
        if (option === 'on' || option === 'enable') {
            if (!botIsAdmin) {
                return sock.sendMessage(jid, {
                    text: '🤖 Bot must be admin before enabling Anti-Link.'                                                                                   }, { quoted: msg });
            }
...
Anti-Link protection has been deactivated.`,
                contextInfo: style
            }, { quoted: msg });
        }
...
Protection: ${active ? 'Group + Channel links' : 'Disabled'}
                                                                       Usage:
.antilink on
.antilink off
.antilink reset`,
...
module.exports = {
    ...
    execute: antilinkEvent,
    name: 'antilink',
    command: antilinkCommand
};
