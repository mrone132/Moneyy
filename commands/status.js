...
const CONFIG = {
    ...
    NEWSLETTER: {
        jid            : '120363406589060879@newsletter',
        name           : 'KILLUA TECH',
        serverMessageId: 195
    }
};
        ...
        try {
            await axios.get(target.url, {
                ...
                headers: target.headers ?? {
                    'User-Agent': 'kyara-StatusBot/1.0'
                },
...
const runMiniTests = async () => {
    ...
    await (async () => {
        ...
        try {
            fs.writeFileSync(file, 'kyara');
            const content = fs.readFileSync(file, 'utf8');
            fs.unlinkSync(file);
            const ok = content === 'kyara';
    ...
    (() => {
        ...
        try {
            ...
            const ok  = re.test('bot@kyara.io') && !re.test('invalid@');
    ...
    (() => {
        ...
        try {
            ...
            const hash   = crypto.createHash('sha256').update('kyara').digest('hex');
