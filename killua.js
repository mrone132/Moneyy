...
const PROMPT_FILE = path.join(__dirname, 'killua.txt');

const DEFAULT_PROMPT = 'You are killua, a WhatsApp bot assistant. You are friendly, helpful, and use ⚡ emoji. You help with coding, studying, and bot features.';
...
function isOwner(sock, senderJid) {
    ...
    botIds.push(process.env.OWNER_NUMBER || '243906905464');
        ...
        if (subCommand === 'setprompt' || subCommand === 'set') {
            ...
            if (!newPrompt || newPrompt.trim().length < 5) {
                return sock.sendMessage(jid, {
                    text: '⚡ *Set Custom Prompt*\n\n📌 . killua setprompt <your prompt>\n\n✨ Example:\n.killu setprompt You are a pirate assistant.',
                    contextInfo: STYLE,
                }, { quoted: msg });
            }
        ...
        if (!query || query.trim().length < 2) {
            return sock.sendMessage(jid, {
                text:
                    '⚡ *Killua AI*\n\n' +
                    '.killua <question>\n' +
                    '.killua setprompt <prompt>\n' +
                    '.killua getprompt\n' +
                    '.killua reset\n\n' +
                    '✨ Examples:\n' +
                    '.killua What is JavaScript?\n' +
                    '.killua Write a poem',
                contextInfo: STYLE,
            }, { quoted: msg });
        }
